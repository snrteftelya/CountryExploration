...
public class LogsController {
    ...
    @Operation(
            ...
            description = "Returns log entries for a specific date. If the Accept header is 'application/octet-stream', a file will be downloaded."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Logs found and returned"),
            @ApiResponse(responseCode = "400", description = "Invalid date format or date is in the future"),
            @ApiResponse(responseCode = "404", description = "No logs found for the specified date or log file not found"),
            @ApiResponse(responseCode = "500", description = "Internal server error while processing log files")
    })
    @GetMapping
    public ResponseEntity<?> downloadOrViewLogs(
            @Parameter(description = "Date in yyyy-MM-dd format", example = "2025-04-24", required = true)
            @RequestParam(name = "date")
            @Pattern(regexp = "\\d{4}-\\d{2}-\\d{2}", message = "Date must be in yyyy-MM-dd format") String date,
            @RequestHeader(value = HttpHeaders.ACCEPT, required = false, defaultValue = MediaType.TEXT_PLAIN_VALUE) String acceptHeader
    ) throws IOException {
        ...
        Files.write(logFile, filteredLogs.getBytes(StandardCharsets.UTF_8), StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);

        if (acceptHeader.contains(MediaType.APPLICATION_OCTET_STREAM_VALUE)) {
            // Клиент хочет скачать файл
            InputStream inputStream = Files.newInputStream(logFile, StandardOpenOption.DELETE_ON_CLOSE);
    ...
    private String getFilteredLogs(String date) throws IOException {
        ...
        try (Stream<String> lines = Files.lines(LOG_PATH)) {
            ...
            String filtered = lines
                    .filter(line -> line.startsWith(date)) // Только строки, начинающиеся с конкретной даты
